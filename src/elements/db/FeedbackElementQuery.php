...
/**
 * @extends ElementQuery<array-key, ElementInterface>
 */
class FeedbackElementQuery extends ElementQuery
{
    ...
    protected array $defaultOrderBy = [
        "feedback_record.dateCreated" => SORT_DESC,
    ];
    ...
    protected function beforePrepare(): bool
    {
        ...
        if ($this->entryId) {
            $this->subQuery->andWhere(
                Db::parseParam("feedback_record.entryId", $this->entryId),
            );
        }

        if ($this->name) {
            $this->subQuery->andWhere(
                Db::parseParam("feedback_record.name", $this->name),
            );
        }

        if ($this->email) {
            $this->subQuery->andWhere(
                Db::parseParam("feedback_record.email", $this->email),
            );
        }

        if ($this->comment) {
            $this->subQuery->andWhere(
                Db::parseParam("feedback_record.comment", $this->comment),
            );
        }

        if ($this->rating) {
            $this->subQuery->andWhere(
                Db::parseParam("feedback_record.rating", $this->rating),
            );
        }
        ...
        if ($this->ipAddress) {
            $this->subQuery->andWhere(
                Db::parseParam("feedback_record.ipAddress", $this->ipAddress),
            );
        }

        if ($this->userAgent) {
            $this->subQuery->andWhere(
                Db::parseParam("feedback_record.userAgent", $this->userAgent),
            );
        }
        ...
        if (!is_null($this->hasComment)) {
            ...
            if ($this->hasComment === true) {
                $this->subQuery->andWhere(
                    Db::parseParam("feedback_record.comment", ":notempty:"),
                );
            }
            ...
            if ($this->hasComment === false) {
                $this->subQuery->andWhere(
                    Db::parseParam("feedback_record.comment", ":empty:"),
                );
            }
        }
