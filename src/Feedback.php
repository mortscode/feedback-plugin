...
class Feedback extends Plugin
{
    ...
    public function init(): void
    {
        ...
        Event::on(
            ...
            static function (DefineHtmlEvent $event) {
                ...
                // Only process actual Entry elements that have sections
                // This excludes Matrix blocks and other nested elements
                if (!($entry instanceof Entry) || !$entry->section) {
                    return;
                }

                // Additional safety check: ensure the entry has a valid ID
                if (!$entry->id) {
                    return;
                }
                ...
                if ($sectionIsSelected) {
                    $reviewUrl = UrlHelper::url(
                        "/admin/feedback/create/" .
                            FeedbackType::Review .
                            "/" .
                            $entryId,
                    );
                    $questionUrl = UrlHelper::url(
                        "/admin/feedback/create/" .
                    ...
                    $event->html .=
                        '<hr/><div class="meta" style="padding: 24px 24px 18px"><h2 class="heading">Add Feedback</h2><div class="flex"><a href="' .
                        $reviewUrl .
                        '" class="btn secondary">Add Review</a><a href="' .
                        $questionUrl .
                        '" class="btn secondary">Add Question</a></div></div>';
                }
            },
        );
        ...
        Event::on(
            ...
            function (RegisterUrlRulesEvent $event) {
                $event->rules["feedback/<entryId:\d+>/<feedbackId:\d+>"] = [
                    "template" => "feedback/_feedback-detail",
                ];
                $event->rules[
                    "feedback/create/<feedbackType:{slug}>/<entryId:\d+>"
                ] = ["template" => "feedback/_feedback-create"];
            },
        );
            ...
            static function (RegisterEmailMessagesEvent $event) {
                $event->messages[] = new SystemMessage([
                    ...
                    "body" => file_get_contents(
                        __DIR__ . "/emails/new_feedback.md",
                    ),
                ]);
            },
        );
        ...
        Event::on(
            ...
            static function (RegisterEmailMessagesEvent $event) {
                $event->messages[] = new SystemMessage([
                    ...
                    "body" => file_get_contents(
                        __DIR__ . "/emails/feedback_response.md",
                    ),
                ]);
            },
        );
        ...
        Event::on(
            ...
            static function (RegisterEmailMessagesEvent $event) {
                $event->messages[] = new SystemMessage([
                    ...
                    "body" => file_get_contents(
                        __DIR__ . "/emails/feedback_approved.md",
                    ),
                ]);
            },
        );
        ...
        // GraphQL Events
        Event::on(Gql::class, Gql::EVENT_REGISTER_GQL_TYPES, function (
         ...
         */
        Craft::info(
            Craft::t("feedback", "{name} plugin loaded", [
                "name" => $this->name,
            ]),
            __METHOD__,
        );
    ...
    public function getCpNavItem(): array
    {
        ...
        $navItem[
            "badgeCount"
        ] = self::getInstance()->feedbackService->getTotalPendingFeedback();
        return $navItem;
    }
    ...
    {
        return Craft::$app->view->renderTemplate("feedback/settings", [
